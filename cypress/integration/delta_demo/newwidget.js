...
describe('example to-do app', () => {
  ...
  it('Delta Vacations Widget page should be displayed', () => {
    ...
    //cy.get('div.input-group').click()

    cy.fixture('navigation').then((testData) => {

       cy.url().should('include','delta-vacations')
       
      // cy.contains('Go Beyond the Flight').should('include.text','Go Beyond the Flight') 
     ...
     // cy.packagesTab('Flight + Hotel (+Car/Transfers)')
      ...
      //cy.get('#hotelResultDetails h1').should('be.visible')
      // cy.get('div#hotelDetailsList button.secondary').then($options => {
      //   const count = $options.length
      //   const randomIndex = Math.floor(Math.random() * count)  
      //   cy.wrap($options.eq(randomIndex)).click({ force: true })
    
      // })
      //cy.get('#loadOutBoundFlightResults').click()

      //Printing Hotel name pending
        // cy.get(' button[class="secondary firstBtn selectable active"]')
        // .parents('div.details-section')
        // .children('div.location-title')
        // .then($hotelname =>{
          
        //     var Value=$hotelname.text()
        //      cy.log(Value)
